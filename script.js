...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (fakturaForm) {
    fakturaForm.addEventListener('submit', e => {
      ...
      const lines = [
        'DzieĹ dobry, wysyĹam dane do przygotowania oferty / faktury:',
        '',
      ...
      lines.push(
        ...
        `MiejscowoĹÄ: ${miejscowosc}`,
        `IloĹÄ ton: ${iloscTon}`,
        formaPlatnosci
          ? `Forma pĹatnoĹci: ${formaPlatnosci === 'przelew' ? 'Przelew przed dostawÄ' : 'GotĂłwka przy dostawie'}`
          : '',
        uwagi ? `Uwagi: ${uwagi}` : '',
        '',
        'ProszÄ o przygotowanie wyceny i terminu dostawy.'
      );

      const text = encodeURIComponent(lines.filter(Boolean).join('\n'));
      const phone = '48793573900';

      if (fakturaSuccess) fakturaSuccess.style.display = 'block';
  ...
  if (calcBtn) {
    calcBtn.addEventListener('click', () => {
      ...
      if (rp) rp.textContent = `${total.toLocaleString('pl-PL')} zĹ`;
      if (rd) {
        rd.textContent = `${pelletName} â˘ ${pallets} palet(y)` + (address ? ` â˘ ${address}` : ' â˘ wycena orientacyjna â dokĹadnÄ cenÄ potwierdzimy po adresie');
      }
      ...
      if (waWrap && waLink) {
        const msgLines = [
          'DzieĹ dobry, proszÄ o wycenÄ pelletu:',
          '',
          `Rodzaj: ${pelletName}`,
          `IloĹÄ: ${pallets} palet(y)`,
          postal ? `Kod pocztowy: ${postalRaw}` : '',
          address ? `Adres dostawy: ${address}` : '',
          '',
          `Szacowana cena z kalkulatora: ${total.toLocaleString('pl-PL')} zĹ`,
          '(wiem, Ĺźe to wycena orientacyjna â proszÄ o dokĹadnÄ ofertÄ)',
        ].filter(Boolean);
